...
cout<<"argvc : "<<argc<<endl;
	...
	for (int k = 0; k < img.size()-1; k++)
	{
		...
		STEP 2: for loop count 1 : image1 will have the 3rd(index:2)image, once the stitching is one with reslutant image from STEP1, the 					resut will be stored in image2

		STEP 3: This loop will run till we finish stichig all the images and the final result will be stored in image2.
